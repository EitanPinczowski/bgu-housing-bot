...
- price_per_room_ils = העלות החודשית לשותף אחד (חדר אחד), ללא חשבונות (ארנונה/ועד/מים).
  שים לב: מחיר יחיד במודעה ("מחיר 2,800 ש״ח", "שכ״ד חודשי 2,950 ש״ח") הוא כמעט תמיד
  שכר הדירה הכולל של כל הדירה — לא המחיר לחדר. אסור להחזיר אותו כמו שהוא.
  קבע לפי הסדר הזה, ועצור בשלב הראשון שמתאים:
  1. אם המודעה נוקבת במפורש במחיר לאדם/לחדר ("1,400 לשותף", "כל חדר 1,200 ש״ח",
     "לכל אחד 900") — החזר את המספר הזה כמו שהוא.
  2. אחרת, אם ידוע מספר הדיירים הכולל בדירה כשהיא מלאה (total_roommates_in_apt) —
     חלק את הסכום הכולל במספר הזה ועגל למספר שלם.
     דוגמה: "דירת 4 חדרים ל2 שותפים... מחיר 2,800" → 2800 / 2 = 1400.
     דוגמה: "דירת 3 חדרים לזוגות... שכ״ד חודשי 2,950" → 2950 / 2 = 1475.
  3. אחרת — החזר null. לעולם אל תחזיר את שכר הדירה הכולל בשדה הזה, גם אם זה המספר
     היחיד במודעה. עדיף null מאשר סכום שלם שיוצג כמחיר לחדר.
